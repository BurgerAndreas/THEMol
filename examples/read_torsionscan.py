...
def read_torsionscan(data_dir):
    ...
    print(f"Loaded TorsionScan CSV with {len(df)} entries.")
    ...
    with h5py.File(h5_path, "r") as f:
        if uuid in f:
            ...
            print("--- Example TorsionScan Data ---")
            print(f"UUID: {uuid}")
            print(f"SMILES: {smiles}")
            print(f"Number of atoms: {len(atomic_numbers)}")
            print(f"Torsion atom indices: {torsion_indices}")

            step_keys = [k for k in grp.keys() if k.startswith("constraint ")]
            print(f"Found {len(step_keys)} constraint steps.")
...
if __name__ == "__main__":
    ...
    DATA_DIR = os.path.join(base_dir, "TorsionScan")
    read_torsionscan(DATA_DIR)
